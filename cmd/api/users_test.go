...
func TestUsers_registerUserHandler(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			wantBody:    fmt.Sprintf("must be at least %d bytes long", repository.PasswordMinLength),
		...
		},
		{
			...
			endpoint: "/v1/users",
			wantCode: http.StatusUnprocessableEntity,
			wantBody: fmt.Sprintf("must be less than %d bytes", repository.PasswordMaxLength),
			requestBody: fmt.Sprintf(`{"firstName":"John","lastName":"Doe","email":"doe@mail.com","password":%q}`,
				strings.Repeat("Pa1#", repository.PasswordMaxLength/4+5)),
		},
		{
			name:        "Empty password",
			endpoint:    "/v1/users",
		...
		},
		{
			...
			endpoint: "/v1/users",
			wantCode: http.StatusUnprocessableEntity,
			wantBody: fmt.Sprintf("must be less than %d bytes", repository.UserFirstNameMaxLength),
			requestBody: fmt.Sprintf(`{"firstName":%q,"lastName":"Doe","email":"doe@mail.com","password":"PASSWORDddd123#"}`,
				strings.Repeat("a", repository.UserFirstNameMaxLength+1)),
		},
		{
			name:        "Empty LastName",
			endpoint:    "/v1/users",
		...
		},
		{
			...
			endpoint: "/v1/users",
			wantCode: http.StatusUnprocessableEntity,
			wantBody: fmt.Sprintf("must be less than %d bytes", repository.UserLastNameMaxLength),
			requestBody: fmt.Sprintf(`{"firstName":"John","lastName":%q,"email":"doe@mail.com","password":"PASSWORDddd123#"}`,
				strings.Repeat("a", repository.UserLastNameMaxLength+1)),
		},
		{
			name:        "Empty Email",
			endpoint:    "/v1/users",
